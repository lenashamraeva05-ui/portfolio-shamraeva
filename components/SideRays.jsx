...
float rayStrength(vec2 raySource, vec2 rayRefDirection, vec2 coord, float seedA, float seedB, float speed) {
  ...
  // Keep the animated shimmer subtle so idle time cannot wash out the dark base.
  float shimmer = 0.5 + 0.5 * sin(cosAngle * seedA + iTime * speed);
  float drift = 0.5 + 0.5 * cos(-cosAngle * seedB + iTime * speed);
  return (0.68 + shimmer * 0.06 + drift * 0.04) *
...
void main() {
  ...
  gl_FragColor = color;
      ...
      const loop = t => {
        ...
        if (animate) uniforms.iTime.value = t * 0.001;
        try {
      ...
      if (animate) {
        ...
        // Render a stable frame when decorative motion is disabled.
        loop(0);
      }
  ...
  }, [isVisible, animate, speed, rayColor1, rayColor2, intensity, spread, origin, tilt, saturation, blend, falloff, opacity]);
